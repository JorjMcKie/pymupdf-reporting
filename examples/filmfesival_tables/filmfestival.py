...
def get_items_by_actor():
    ...
    for actor_row in actor_rows:
        ...
        actor_films = actor_info.get(actor, [])
        
        cursor_film = cursor_films.execute(select_films % film)
        film_year = cursor_film.fetchall()
        actor_films.append(f"{film} ({film_year[0][0]})")
        actor_info[actor] = actor_films
...
actor_items = Table(
    report=report,
    html=ACTOR_HTML,
    fetch_rows=get_items_by_actor,
    top_row="toprow",
)

report.sections = [film_items, actor_items]
report.header = [header]
report.run("filmfestival3.pdf")
