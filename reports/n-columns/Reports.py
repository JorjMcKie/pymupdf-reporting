...
class NColumnsReport(object):
    def __init__(self, mediabox, margins=None, columns=10):
        self.mediabox = mediabox
        if margins == None:
            self.where = self.mediabox + (36, 36, -36, -50)
        else:
    ...
    def run(self, filename):
        ...
        while more:  # loop until all input text has been written out
            ...
            COLS = self.columns  # layout: N cols 1 row
            ROWS = 1 # default
            TABLE = fitz.make_table(self.where, cols=COLS, rows=ROWS)
            ...
            while more_cell: # loop until it reach out max column count in one page
                ...
                if more:  # so check this status first
                    more, filled = self.sections[section_index].story.place(where)
                    self.sections[section_index].story.draw(dev, None)
                print(cell_index, CELL_LENGTH)
                print(more, where.y0, filled[3], self.where.y1)
                if filled[3] < self.where.y1: # check and add section/block
                    section_index += 1
                    where.y0 = filled[3]

                if more and filled[3] >= self.where.y1: # check and select next column
                    cell_index += 1

                if cell_index == CELL_LENGTH: # check whether one page is completed
                    more_cell = False
                ...
                if section_index >= len(self.sections): # check completion of PDF
                    more = False
